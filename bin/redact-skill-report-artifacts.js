...
const { redactSkillReportArtifacts } = require('../src/store');
...
function parseArguments(argv) {
  ...
  throw new Error('Usage: redact-skill-report-artifacts.js [--apply]');
}

async function main() {
  const { apply } = parseArguments(process.argv.slice(2));
  const result = await redactSkillReportArtifacts();
